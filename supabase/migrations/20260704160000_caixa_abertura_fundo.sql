-- Fundo de caixa informado na abertura (troco inicial).

alter table public.caixa_lancamentos
  add column if not exists valor_fundo numeric(14, 2) null;

alter table public.caixa_lancamentos
  drop constraint if exists caixa_lancamentos_valor_fundo_nao_neg;

alter table public.caixa_lancamentos
  add constraint caixa_lancamentos_valor_fundo_nao_neg
  check (valor_fundo is null or valor_fundo >= 0);

comment on column public.caixa_lancamentos.valor_fundo is
  'Fundo em dinheiro informado na abertura do caixa (troco).';

alter table public.caixa_movimento
  add column if not exists id_lancamento_caixa bigint null
    references public.caixa_lancamentos (id) on delete set null;

create unique index if not exists caixa_movimento_id_lancamento_caixa_uq
  on public.caixa_movimento (id_lancamento_caixa)
  where id_lancamento_caixa is not null;

comment on column public.caixa_movimento.id_lancamento_caixa is
  'Lançamento de abertura/fechamento de caixa vinculado ao movimento (ex.: fundo na abertura).';

-- Resumo do fechamento: incluir fundo de caixa (troco) no esperado de dinheiro.
...
as $$
  with base as (
    ...
    where cm.tipo_entrada in ('taxa_agendamento', 'fundo_caixa')
...
comment on function public.caixa_resumo_pagamentos_dia is
  'Totais quitados no dia: pagamentos, taxas e fundo de caixa na abertura.';
