-- Corrige forma de pagamento das taxas (PIX, cartão, boleto, dinheiro) e alinha resumo do caixa.

-- Atualiza movimentos genéricos a partir do billingType salvo na taxa Asaas.
update public.caixa_movimento cm
set forma_pagamento = sub.nova_forma
from (
  select
    cm2.id,
    case upper(
      coalesce(
        t.asaas_resposta -> 'payment' ->> 'billingType',
        (
          select p ->> 'billingType'
          from jsonb_array_elements(
            case
              when jsonb_typeof(t.asaas_resposta -> 'data') = 'array'
                then t.asaas_resposta -> 'data'
              else '[]'::jsonb
            end
          ) as p
      ...
      when 'RECEIVED_IN_CASH', 'CASH' then 'Dinheiro'
      else cm2.forma_pagamento
    end as nova_forma
  from public.caixa_movimento cm2
  inner join public.agendamento_taxa_rede t on t.id = cm2.id_taxa_rede
  where cm2.tipo_entrada = 'taxa_agendamento'
    and nullif(trim(t.asaas_payment_link_id), '') is not null
) sub
where cm.id = sub.id
  and sub.nova_forma is not null
  and sub.nova_forma <> cm.forma_pagamento
  and cm.forma_pagamento in ('Link pagamento', 'Taxa agendamento', '—');
...
as $$
  with base as (
    ...
    where cm.tipo_entrada = 'taxa_agendamento'
...
comment on function public.caixa_resumo_pagamentos_dia is
  'Totais quitados no dia: pagamentos de atendimentos + taxas de agendamento (caixa_movimento, por forma real).';
