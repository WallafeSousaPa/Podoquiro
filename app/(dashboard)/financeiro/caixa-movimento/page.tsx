...
export default async function CaixaMovimentoPage() {
  const session = await getSession();
  if (!session) redirect("/login");

  const empresaId = Number(session.idEmpresa);
  if (!Number.isFinite(empresaId) || empresaId <= 0) redirect("/inicio");
        ...
        <div className="container-fluid">
          <div className="row mb-2">
            <div className="col-sm-6">
              <h1 className="m-0 text-dark">Caixa Movimento</h1>
            </div>
            <div className="col-sm-6">
              <ol className="breadcrumb float-sm-right">
                ...
                <li className="breadcrumb-item active">Caixa Movimento</li>
              </ol>
            </div>
          </div>
        </div>
