...
export async function GET(request: Request) {
  ...
  if (!DATA_RE.test(inicio) || !DATA_RE.test(fim)) {
    return NextResponse.json(
      { error: "Parâmetros inicio e fim inválidos (use YYYY-MM-DD)." },
      { status: 400 },
    );
  }
  ...
  const dInicio = new Date(yi, mi - 1, di);
  const dFim = new Date(yf, mf - 1, df);
  if (dFim < dInicio) {
    return NextResponse.json(
      { error: "A data fim deve ser igual ou posterior ao início." },
      { status: 400 },
    );
  }
  const diffDias =
    Math.round((dFim.getTime() - dInicio.getTime()) / (24 * 60 * 60 * 1000)) + 1;
  if (diffDias > MAX_DIAS_INCLUSIVOS) {
    return NextResponse.json(
      { error: `Período máximo de ${MAX_DIAS_INCLUSIVOS} dias.` },
      { status: 400 },
    );
  }
  ...
  const agendamentos = (agRows ?? []).map((r) => {
    ...
    const paciente_nome = pac ? nomeExibicaoPaciente(pac, pid) : "Paciente";
